...
function avgAge(persons) {
  const sum = persons.reduce(function (accumulator, currentValue) {
...
function longestName(persons) {
  ...
  for (let i = 0; i < persons.length; i++) {
    fullNames.push(persons[i].name.first + ' ' + persons[i].name.last);
  }
  console.log(fullNames);


  const longest = fullNames.reduce(function (previousValue, currentValue) {
    let longestName = previousValue;
    if (currentValue.length > longestName.length) {
      longestName = currentValue;
    }
    return longestName;
  });
  console.log(longest);
...
*/
function repeatChar(theString, theChar) {
  let counter = 0;
  const myCounter2 = Array.prototype.reduce.call(theString, (previousValue, currentValue) => {
    if (currentValue == theChar) {
      counter++;
    }
    return counter;
  });
...
function evenOnly(arr) {
  const newArray = arr.filter((number) => {
    return number%2 == 0;
  });
...
function evenIndexOddLength(arr) {
   const newArray = arr.filter((str) => {
    return (str.length % 2 != 0) && (arr.indexOf(str) % 2 == 0);
   });
   console.log(newArray);
}
var strings= ["alex","mercer","madrasa","rashed2","emad","hala", "madrasa",];
...
*/
function olderThan(persons,x) {
  let newArray = persons.filter(obj => {
    return obj.age > x;
  });
  console.log(newArray);
}
...
function shorterThan(strings,x) {
